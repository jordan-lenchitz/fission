...
if hz
    if ~isempty(candidates_1)
        plot(erb2hz(aerb), b, '.', erb2hz(candidates_0(:,1)), b(test_inds), '+r', erb2hz(candidates_1(:,1)), b(ttest_inds), '*g')
    else
        plot(erb2hz(aerb), b, '.', erb2hz(candidates_0(:,1)), b(test_inds), '+r')
    end
    xlabel('Frequency (Hz)')
else
    if ~isempty(candidates_1)
        plot(aerb, b, '.', candidates_0(:,1), b(test_inds), '+r', candidates_1(:,1), b(ttest_inds), '*g')
    else
        plot(aerb, b, '.', candidates_0(:,1), b(test_inds), '+r')
    end
